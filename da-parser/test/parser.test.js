...
`;
  ...
  it('serializes links innermost so EDS decorateButtons works', async () => {
    // The `link` mark must nest inside em/strong (etc.) so buttons authored as
    // *__[text](url)__* round-trip to `<em><strong><a>` — the order EDS button
    // decoration relies on (link.closest('em')). Regression guard for the mark order.
    const cases = [
      ['<main><div><p><a href="/x"><em>Secondary</em></a></p></div></main>',
        '<p><em><a href="/x">Secondary</a></em></p>'],
      ['<main><div><p><a href="/x"><strong>Primary</strong></a></p></div></main>',
        '<p><strong><a href="/x">Primary</a></strong></p>'],
      ['<main><div><p><a href="/x"><em><strong>Both</strong></em></a></p></div></main>',
        '<p><em><strong><a href="/x">Both</a></strong></em></p>'],
    ];
    cases.forEach(([input, expectedFragment]) => {
      const yDoc = new Y.Doc();
      aem2doc(input, yDoc);
      expect(doc2aem(yDoc)).to.include(expectedFragment);
    });
  });
